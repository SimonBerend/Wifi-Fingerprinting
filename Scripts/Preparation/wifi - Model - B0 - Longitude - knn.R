...
# train & test set ------------------------------------------------------------
# subset and feature selection: WAPS and LONGITUDE
wifi_train <- wifi %>% filter(BUILDINGID == 0) %>% select(starts_with("WAP"), LONGITUDE) 
wifi_test <- wifi_val %>% filter(BUILDINGID == 0) %>% select(starts_with("WAP"), LONGITUDE)
...
# model_B0_longitude_knn <- long_model
# saveRDS(model_B0_longitude_knn, file = "Data/Clean/model_B0_longitude_knn.rds")
