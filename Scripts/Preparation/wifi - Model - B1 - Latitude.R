...
wifi_train <- wifi %>% filter(BUILDINGID == 1) %>% select(starts_with("WAP"), LATITUDE) 
wifi_test <- wifi_val %>% filter(BUILDINGID == 1) %>% select(starts_with("WAP"), LATITUDE)
...
predict_B1_lat <- predict_long
 

model_B1_latitude_knn <- long_model
saveRDS(model_B1_latitude_knn, file = "Data/Clean/model_B1_latitude_knn.rds")
