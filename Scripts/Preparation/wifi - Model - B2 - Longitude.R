...
wifi_train <- wifi %>% filter(BUILDINGID == 2) %>% select(starts_with("WAP"), LONGITUDE) 
wifi_test <- wifi_val %>% filter(BUILDINGID == 2) %>% select(starts_with("WAP"), LONGITUDE)
...
# put predictions next to observations
modelvalues <- data.frame(obs = wifi_test$LONGITUDE, pred = predict_long)

m1 
defaultSummary(modelvalues)
# summary(modelvalues)

# model_B2_longitude_knn <- long_model
saveRDS(model_B2_longitude_knn, file = "Data/Clean/model_B2_longitude_knn.rds")
