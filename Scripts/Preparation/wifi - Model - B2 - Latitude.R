...
wifi_train <- wifi %>% filter(BUILDINGID == 2) %>% select(starts_with("WAP"), LATITUDE) 
wifi_test <- wifi_val %>% filter(BUILDINGID == 2) %>% select(starts_with("WAP"), LATITUDE)
...
wifi_train[,building_useful_waps][wifi_train[,building_useful_waps] < 0.83] <- 0
wifi_test[,building_useful_waps][wifi_test[,building_useful_waps] < 0.83] <- 0
...
model_B2_latitude_knn <- long_model
saveRDS(model_B2_latitude_knn, file = "Data/Clean/model_B2_latitude_knn.rds")
